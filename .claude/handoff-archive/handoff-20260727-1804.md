...
> 最後更新：2026-07-26 21:11 — 導入 ASP.NET Core Identity：登入 / 登出與員工 / 主管兩種角色
...
- 兩種角色：員工 / 主管
- 初始帳號種子 `manager`、`employee`（登入帳號用 email 前綴，不是完整 email）
...
- **權限工作只做到登入與角色**：申請人、審核人、結清處理人、不採用處理人**仍是手動輸入**，還沒接上登入者。
- 還沒限制誰可以核准 / 退回 / 拒絕，也還沒限制誰可以建預支款 / 結清 / 標記不採用。
- 員工還看得到所有人的報銷單，尚未依登入者過濾。
- 沒有使用者管理 UI；新帳號目前只能靠種子資料或直接改 DB。
- 沒有附件或發票照片上傳。
...
- 桌機本機 DB 已套用最新 migration `20260726122529_AddIdentityTables`。
...
- `identity-and-authentication.md`（授權規則做完後要回來補一段）
...
Identity 的分層落點（新增，理由詳見 `docs/architecture/identity-and-authentication.md`）：
...
- **Domain 不認識 `ApplicationUser`**；之後要記「是誰做的」時只存 `Guid` 形式的 UserId，比照跨 aggregate 用 ID 參照的規則。
...
### 進行中：權限工作（分 5 階段，已完成 1–2）

1. ✅ Identity 基礎建設
2. ✅ 登入 / 登出
3. ⬜ **把手動輸入的人名改成登入者**——報銷單申請人、審核人、結清處理人、不採用處理人。作法已定調：Domain 新增 `Guid` 形式的 UserId 欄位，**同時保留現有姓名字串當「當時的姓名快照」**（人事後改名，歷史紀錄不該跟著變），舊資料 UserId 留 null。會動到 Domain 與 migration。
4. ⬜ 授權規則——主管才能核准 / 退回 / 拒絕，也才能建預支款、結清、標記不採用。
   - **做到這裡要先問 Amber**：兩種角色沒有系統管理員，那誰能新增 / 停用使用者帳號？（`ApplicationUser` 已預留 `IsActive` 欄位。）
5. ⬜ 員工只看自己的報銷單——列表與詳情依登入者過濾，主管看得到全部。

### 暫緩，等權限做完再回頭

**「預支款已結清」後是否拿掉修改功能。** Amber 原本決定要拿掉，但接著判斷「先把權限補起來再回頭看，方向會更明確」——因為「誰能改」會影響「還能不能改」。回頭時要決定的具體範圍：

- 已結清後要鎖的：預支款的用途 / 預支金額、結清紀錄的結清日期 / 金額 / 處理人 / 備註。
