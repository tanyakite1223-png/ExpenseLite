...
> 最後更新：2026-08-05 23:13 — 預支款模型經 Amber review 後推翻重做，**本 session 只做決策、未動任何 code**

---

## ⚠️ 下一個 session 開場要做的事

**執行「預支款模型收斂 + 付款方式三選一」**，計畫已完整定案，見〈待執行：預支款模型收斂〉。動手前不需要再討論方向，但第一步是**不可逆的清空資料庫**，要先跟 Amber 確認一次再跑。

原本排在這裡的「手動驗收階段 4a + 4b」**往後挪**：4b 的用途類型那半即將被移除，現在測沒有意義。等收斂做完、資料清乾淨之後，再用新資料一次驗收 4a 授權規則與新的預支款模型。

---

## 待執行：預支款模型收斂

> 2026-08-05 Amber review 後決定。理由與過程見〈已定案的設計決策 / 預支款與零用金〉。
> **這是下一個 session 的主線工作，做完之前不要往階段 5 走。**

### 為什麼要重做

4b 把「零用金」做成「多人可共用的預支款」，這是建模錯誤。零用金不是預支給某個人的錢，是一筆由保管人看管的定額基金；把它塞進預支款，核對算式（差額 = 已核准報銷 − 預支金額）算出來的語意就是錯的——大家報滿額度時系統說「已結清」，現實中那正是該補撥回抽屜的時刻。

### 執行步驟

1. **`pg_dump` 備份**到 repo 外（scratchpad），保留 Amber 手 key 的真實流程樣本資料
2. **清空業務資料**——`expense_reports`（含明細、審核紀錄）、`cash_advances`（含結清紀錄）、`projects`；**保留 Identity 的使用者與角色**（不然要重設密碼、重跑 seed）
3. **收斂預支款**：刪掉 `Domain/CashAdvances/CashAdvanceUsage.cs`；`CashAdvance.Create()` 拿掉 usage 參數、移除 `Usage` 屬性；`CanBeReferencedBy` 從兩個分支縮成一行 `cashAdvance.PayeeUserId == viewer.UserId`；Command / DTO / ViewModel 拿掉 `Usage`
4. **Web 層清理**：`Web/Views/CashAdvances/` 的 Create / Edit / Details / Index 拿掉用途類型的顯示與下拉、零用金說明區塊、建立時的 `WarningMessage`；`Web/Views/ExpenseReports/` 的 Create / Edit / Details 拿掉零用金標示
5. **付款方式三選一**：`ExpensePaymentMethod` 的 `CashAdvance` 改名 `PersonalAdvance`、新增 `PettyCash`；`ExpenseReport.EnsureBasicInfoIsValid` 的兩條對稱規則併成一條（只有個人預支要綁預支款，其餘都不綁）
6. **migration**：drop `cash_advances.usage` 欄位
7. **更新 `docs/architecture/cash-advance-reconciliation.md`**：拿掉領款人 / 用途類型那兩節，改寫成「為什麼零用金不是預支款」
8. 收斂完成後，才做 4a 授權規則與新模型的手動驗收

### 收斂後的 invariant 長相（步驟 5）

```csharp
// 只有個人預支要綁預支款；員工墊款與零用金支付都不綁。
...
加了一個付款方式，規則反而從兩條變一條——這是模型對了的訊號。

### 注意事項

- `payment_method` 在 DB 存的是**字串**（`ExpenseReportConfiguration.cs:50`），不是 int。所以 enum 改名會動到既有資料的字串值——**這正是先清空資料庫的主要理由之一**，清乾淨就不用寫 UPDATE 遷移。
- 4b 的成果**只有一半被拿掉**。領款人存 `PayeeUserId` + 姓名快照那半是對的、完整保留；`Usage` 那半是專門為了支撐「多人共用」而生的，跟著問題一起消失。
- 建議分兩個 commit：`refactor: 預支款收斂為個人預支，移除用途類型` 與 `feat: 報銷單付款方式新增零用金支付`。

### 為什麼要清空資料庫（步驟 2 的完整理由）

- `payment_method` 存字串，enum 改名有資料就要寫遷移（見上）。
- **階段 5 用現有資料測不出來**：21 張舊報銷單 `applicant_user_id` 全是 null，沒有人是申請人。做「員工只看自己的報銷單」時，這批資料會對所有員工隱形、只有主管看得到，驗證不了「A 看得到自己的、看不到 B 的」。
- 7 筆舊預支款 `payee_user_id` 全 null，收斂後同樣只能當唯讀歷史，測不到任何新規則。
- 舊資料現在會在核對表顯示語意錯誤的數字（例如「公司補付 780 給 Candy」），手動測試時只製造混淆。
...
> ⚠️ 預支款與付款方式相關的項目即將依〈待執行：預支款模型收斂〉改寫，以下描述的是**收斂前**的現況。
...
- `TempData` 訊息目前有三種：`SuccessMessage`、`ErrorMessage`、`WarningMessage`。（`WarningMessage` 目前唯一的用途是 4b 的零用金提示，收斂後會沒有使用者；機制留著，之後有「不是成功、是提醒」的情境可以再用。）
...
- `cash-advance-reconciliation.md`（**收斂時要改寫**，見〈待執行〉步驟 7）
...
- `CashAdvanceAppService.CanBeReferencedBy(...)`——**這不是授權而是模型規則**：換一個角色答案不會變（主管也不能拿別人的預支款報自己的單）
...
- **「已核准報銷」是每次讀取時即時重算的，沒有鎖定點。** 一筆已結清的預支款，之後若再被新的報銷單引用並核准，差額會再度變動，原本已結清的紀錄方向可能就反了。目前只在「修改結清紀錄」時擋（`CashAdvanceAppService.cs:297`），沒擋住「已結清後又被引用」這個入口。收斂成一人一筆之後風險變小，但問題還在。
...
### 預支款與零用金（2026-08-05 重新定案，推翻 4b）

- **零用金不是預支款。** 預支款是「公司先把錢給某個人，之後多退少補」，會計上是對那個人的應收款，**一人一筆、不可能共用**。零用金是「一筆由保管人看管的定額基金」——大家從抽屜拿錢花，花掉的憑單據補回抽屜，基金額度不變、也沒有結束的一天。兩者放進同一條核對算式必然算錯。
- **推翻 2026-07-31 的「預支款區分個人預支 / 零用金」決策。** 那個設計是想用一個 `Usage` 欄位讓零用金能被多人引用，但它讓「差額 = 已核准報銷 − 預支金額」對零用金失去意義：大家報滿額度時系統判定「已結清」，現實中那正是該補撥回抽屜的時刻，方向剛好相反。**不要再提議把零用金做回預支款。**
- **預支款收斂成一人一筆的個人預支**，只有領款人本人的報銷單能引用。
- **零用金改成一種「付款方式」**，不是一筆資料。付款方式三選一，依據是**錢從哪來**：
  - **員工墊款**——設計師自己的錢先出，公司要付錢還他
  - **個人預支**——公司事先給的錢，用報銷單沖抵、多退少補；**必須綁一筆預支款**
  - **零用金支付**——錢從抽屜出的，設計師當場就拿到現金了，公司不用再付他；**不綁任何預支款，只是標記**
- **不設金額門檻。** 大筆走墊款、小筆走零用金只是現實的相關性（抽屜裡就那點現金），不是規則。綁死會卡住「抽屜空了所以自己先付 300」這種正常情況。
- **付款方式留在報銷單層級，不下沉到明細。** 設計師一到兩週交一次單，一疊裡可能混了兩種來源——處理方式是**分成兩張報銷單**。理由：十人公司分兩張不是負擔，而且分開對出納更清楚（「這張要付你 8,500」「這張抽屜已經付掉了 3,200」）；反之若下沉到明細，個人預支要綁的預支款就不知道該掛在哪，整套核對邏輯要改寫成按明細加總。
- **零用金支付的報銷單一樣要走核准，不按金額分流。** 錢雖然當場付掉了，單據還是得主管認可，不然抽屜變成沒人看的黑洞。批次交單本來就是一次審一疊，成本很低；設門檻反而多養一套規則。
...
- **但業務要的資料系統已經給得出來**：專案詳情頁可查該專案全部相關報銷單，業務開請款單時打開專案就看得到代墊了什麼、多少錢。**系統提供資料，不接管流程。**
...
**現有資料（2026-08-05 查核，共 11 筆預支款）——全部即將依〈待執行〉步驟 2 清空**

- **7 筆舊預支款**：領款人是手打字串（Candy、Belle、Amber ×2、Amy ×2、Tone），`payee_user_id` 全為 null，`usage` 由 migration backfill 成 `PettyCash`。其中 **3 筆確實是多人引用**（Candy 那筆有 Belle / Will / Amber 三人開單、Belle 那筆有 Amber / Candy / Tome 三人、Amber 其中一筆有 Amy / Amber 兩人），而且報銷總額都遠超過預支金額。**這批資料是「零用金確實被多人共用」的實證**，也是這次推翻 4b 的直接證據來源。
- **3 筆 4b 測試預支款**（2026-08-03 由 AI 建立），用途以「（測試）階段4b」開頭，無任何引用。
- **1 筆 2026-08-05 建立的「預支零用金」**（王主管、1,000、`payee_user_id` 有值），有陳員工的兩張報銷單引用（450 已核准、一張 0 元草稿）。應為 Amber 手動測試 4a / 4b 留下的。
- 報銷單方面：21 張舊報銷單 `applicant_user_id` 為 null（早於登入功能），另有標題以「（測試）」開頭的驗證用單據。
...
- **用 PowerShell 直接把含中文的 SQL 傳給 `psql -c` 會噴 `invalid byte sequence for encoding "UTF8"`。** 要下含中文的查詢，先 `Set-Content -Encoding UTF8` 寫成 `.sql` 檔再用 `psql -f` 執行；或者查詢裡只用英文別名。
...
4. **授權規則**
   ...
   - ⚠️ **4b 預支款領款人模型**——領款人存 UserId 那半保留；用途類型那半**即將移除**，見〈待執行：預支款模型收斂〉
5. ⬜ 員工只看自己的報銷單——列表與詳情依 `ApplicantUserId` 過濾，主管看得到全部
6. ⬜ 帳號自助註冊與使用者管理——細節見下方

**順序**：先做完〈待執行：預支款模型收斂〉，再走階段 5。
...
### 收斂完成後的下一個設計主題：已核准報銷單更正 / 取消核准

> Amber 在 2026-08-05 這個 session 兩度提起，**不是可有可無的候補項目**。
> **但刻意不跟收斂併在一起做**：收斂是重構（把錯的模型改對），這是新流程（在 `ExpenseReport` 上開新的狀態轉換），性質不同，混做會分不清問題出在哪。而且在收斂後的模型上討論才準——每筆預支款只對一個人時，核對數字更精確，「已核准報銷變動會讓結清方向翻掉」這個裂縫也更好講。
...
- **被推翻的決策要留一句「為什麼推翻」**（見〈預支款與零用金〉），否則下一個 session 很可能重新提議一次同樣的設計。
