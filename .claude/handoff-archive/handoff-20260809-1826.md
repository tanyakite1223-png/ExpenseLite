...
> 最後更新：2026-08-09 16:34 — 完成「已核准報銷單作廢」與「草稿硬刪 / 退回單軟刪 + 復活」流程；程式碼實作完成、`dotnet build` 通過，**但尚未手動驗過**（Amber 為了節奏考量把測試留給下一個 session）

---

## ⚠️ 下一個 session 開場要做的事

**開場第一件事：驗證本 session 未測的實作**。「已核准報銷單作廢」+「草稿硬刪」+「退回單軟刪 + 復活」的整套已在 2026-08-09 寫完，build 過 0 warning / 0 error，但沒開瀏覽器手動試過。驗證路徑列在下方〈驗證清單〉。

**驗證前必須先跑 fixture SQL**——現有 DB 只有一張一般支出的 fixture，作廢流程需要「已核准 + 綁預支款 + 有結清紀錄」的組合。SQL 由 Claude 在上個 session 對話裡提供、**刻意不落檔進 repo**（Amber 決策）；若找不到請直接跟 Claude 要一份。跑完會多：1 個進行中的專案「品牌識別更新案 - 綠地科技」+ 1 筆 Butter 領款 5000 元的預支款 + 1 張 Butter 送、Admin 核准的專案支出報銷單（3500 元）+ 1 筆結清紀錄（員工繳回 500）。

**驗證後**：測完若都 OK，剩下的暫緩項目「預支款已結清後是否鎖定修改」還在（見〈暫緩項目〉）；發現 bug 就先修 bug、再回來。

「主管自我核准缺口」已於 2026-08-08 收乾淨（見〈已完成功能 / 報銷單〉的「審核人 ≠ 申請人」條目）——當時確認這間公司實際上有兩位主管（老闆 + 老闆娘，均有登入帳號、老闆娘實際做審核），不是原本 handoff 假設的「只有一位主管」死結，直接補「審核人 ≠ 申請人」就夠了。**這個前提要記著**：如果哪天公司只剩一位在動的主管，他自己的單就核不了，那時候才需要重新設計（「主管的單免審」或「必須另一位主管核」都得先決定制度）。

帳號那一塊也不必重新討論：階段 6 做完之後，2026-08-08 又把自助註冊整個拿掉、改成主管建帳號（理由見〈已定案的設計決策 / 帳號生命週期〉的推翻紀錄）。**不要提議把自助註冊做回來**，那個兩難沒有 SMTP 就解不掉。

---

## 驗證清單（下一個 session 開場的實測項目）

> 全部路徑假設 fixture SQL 已跑。用 Butter 是為了貼近實際情境（員工才是預支款常見的領款人）。

**A. 草稿硬刪**
1. 用 Butter 登入、建一張新草稿（隨意內容）
2. 詳情頁按「刪除草稿」→ JS confirm → 確定
3. 預期：跳回列表、看到成功訊息、列表已看不到那張單

**B. 退回單軟刪 + 復活**
1. 用 Butter 登入、送審某張單（若沒有可先建一張並加明細後送審）
2. 登出、換 amber 或 Admin 登入、把那張單「退回」（要填理由）
3. 登出、換回 Butter、進那張退回單詳情頁按「取消此單」→ JS confirm → 確定
4. 預期：跳回詳情頁看到成功訊息、頂部灰色 alert 說「已取消」、狀態徽章顯示「已取消」
5. 回列表：預設看不到那張單；勾「顯示已取消」→ 出現
6. 進詳情頁按「復活」→ 確認回到退回狀態，右側新增明細表單重新可用

**C. 主管作廢已核准單（含兩個提示時機）**
1. 用 Admin 登入、進 fixture 那張已核准的專案支出報銷單詳情頁
2. 預期右側「作廢處理」form 有黃色警告「這張單牽動了 1 筆對應預支款上的已計入結清紀錄」
3. 填作廢原因、按「作廢」→ JS confirm「作廢後不可撤銷…」→ 確定
4. 預期：跳回詳情頁看到成功訊息、狀態徽章紅色「已作廢」、頂部黃色 alert「此單已作廢…仍有 1 筆結清紀錄」；審核紀錄多一筆「作廢」動作
5. 跳到 fixture 那筆預支款詳情頁（`/CashAdvances/Details/22222222-2222-2222-2222-222222222222`）
6. 預期：頂部黃色 alert「這筆預支款有 1 張已被作廢的報銷單…」；核對表：`已核准報銷` 應該變 0、`差額` 從 -1500 變 -5000（員工要繳回 5000）；結清紀錄的 500 元仍在（不會自動動）

**D. 側門測試**（權限）
1. Butter 用瀏覽器直接 POST `/ExpenseReports/Delete/{amber 或別人的單 ID}` → 應 403 或看到「只有申請人可以管理自己的報銷單」
2. amber 登入、直接 POST `/ExpenseReports/Void/{amber 自己送的單 ID}` → 應被 `EnsureReviewerIsNotApplicant` 擋下、DomainRuleViolation
...
- 已實測繞過畫面的側門：直接 POST 送審別人的草稿、直接 POST 結清、直接開修改頁、直接打網址開別人報銷單的詳情頁、員工直接 POST `/Users/Activate`、直接開 `/Users/ResetPassword`，都被擋下。**本次新增的 Delete / Cancel / Restore / Void 側門尚待驗證**（見〈驗證清單 D〉）
...
- **「未完成報銷單」判定全面改成 whitelist**（2026-08-09）：原本用 blacklist 排除 Approved/Rejected，改成明列 Draft/Submitted/Returned。三處都改：`EfExpenseReportRepository.CountUnfinishedProjectReportsAsync` / `HasUnfinishedProjectReportsAsync`、`ProjectAppService.CountUnfinishedReports`。動機：新增狀態時**預設不算未完成**比較安全（Voided/Cancelled 都不流動了，就不該算未完成）
...
- **「已核准報銷」即時重算的缺口已補一半**：2026-08-09 加了作廢流程之後，主管有正式的路把已核准單撤下來（會即時扣掉那張單的金額 + 顯示提示提醒去處理結清紀錄）。**但仍沒鎖定「已結清後又被新單引用並核准」這條路**——一筆已結清的預支款，若領款人本人再開新單並被核准，差額會再度變動、原本已結清的紀錄方向可能就反了。目前只在「修改結清紀錄」時擋（`CashAdvanceAppService`），沒擋住入口。收斂成一人一筆之後風險已小（只有領款人本人開得了新單），但問題還在——參考解法可能是「已結清後鎖定引用」或「已結清後鎖定該預支款的新單建立」，見〈暫緩項目〉。
...
- **「已核准報銷單選錯預支款」或「已核准後要取消該報銷單」**現在有正式流程處理了（見〈已定案的設計決策 / 報銷單生命週期擴充〉的作廢動作）——原本這條寫「不屬於結清紀錄不採用的範圍」的邊界依然成立，`IsVoided` 只長在 `CashAdvanceSettlementRecord` 上；主管作廢已核准報銷單後，仍需要手動判斷相關結清紀錄要不要標記不採用。
...
**已存在的 fixture**：
- 一張 2026-08-08 驗「審核人 ≠ 申請人」時留下的一般支出報銷單 `d4365203-66a3-4e4b-9602-74332de1ddcf`：員工墊款、金額 300、申請人 amber、審核人 Admin、狀態已核准。

**下 session 開場需要跑的 fixture SQL**（2026-08-09 給的、未落檔進 repo）：一個進行中的專案「品牌識別更新案 - 綠地科技」+ 一筆 Butter 領款 5000 元的預支款 + 一張 Butter 送 Admin 核准的專案支出報銷單（3500 元）+ 一筆結清紀錄（員工繳回 500）。目的是給「已核准報銷單作廢」流程一組完整的驗證用資料。**若找不到 SQL 內容，直接跟 Claude 要一份**——內容決策：Butter 而非 amber 是為了貼近實際情境（員工才是預支款常見的領款人），也能演示「主管作廢員工的已核准單」。固定 UUID：Project `11111111-...`、CashAdvance `22222222-...`、ExpenseReport `33333333-...` 等，方便事後精準刪除。
...
**6 之後的修正（2026-08-08 & 2026-08-09）**：

- ✅ 拿掉自助註冊改成主管建帳號、帳號狀態收斂為兩態（2026-08-08）。理由見〈已定案的設計決策 / 帳號生命週期〉。
- ✅ 補上「審核人 ≠ 申請人」規則（2026-08-08）。主管自己送的單只能由另一位主管審。前提是這間公司實際上有兩位主管（老闆 + 老闆娘），若哪天只剩一位在動的主管，需重新設計（見〈⚠️ 下一個 session 開場要做的事〉）。
- ✅ **報銷單生命週期擴充**（2026-08-09）：主管作廢已核准單 + 申請人硬刪草稿 + 申請人軟刪退回單 + 復活。詳見〈已定案的設計決策 / 報銷單生命週期擴充〉與 `docs/architecture/expense-report-lifecycle.md`。**尚未手動驗過**（見〈⚠️ 下一個 session 開場要做的事〉與〈驗證清單〉）。

### 暫緩項目：「預支款已結清」後是否拿掉修改功能

Amber 原本決定要拿掉，當時判斷「先把權限補起來再回頭看」——**權限、帳號、報銷單作廢流程都做完了，前提條件已經消失，可以隨時回頭決定。** 要決定的具體範圍：

- 已結清後要鎖的：預支款的用途說明 / 預支金額、結清紀錄的結清日期 / 金額 / 備註。
- **尚未決定**：「標記為不採用」要不要一起鎖。保留它等於留一條更正出口（不採用後回到未結清就能重做）；一起鎖則是完全凍結，打錯字只能等主管把相關已核准報銷單作廢（現在已有這條路，見 2026-08-09 完成的〈報銷單生命週期擴充〉）。
- 注意 `已結清` 包含兩種情況：差額為 0（根本不需要結清紀錄）、以及尚待結清為 0（結清紀錄補齊）。
- **相關的仍在的缺口**（見〈後續可能優化〉最後一條）：即使加了作廢，仍沒鎖定「已結清後又被新單引用並核准」這條路。要不要一起處理，可在暫緩項目討論時一併決定。
