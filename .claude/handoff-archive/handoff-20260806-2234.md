...
> 最後更新：2026-08-06 20:07 — 預支款模型收斂 + 付款方式三選一**已完成並 commit**；DB 已清空重建

---

## ⚠️ 下一個 session 開場要做的事

**做階段 5：員工只看自己的報銷單**（列表與詳情依 `ApplicantUserId` 過濾，主管看全部）。這是權限工作 6 個階段裡唯一還沒動的核心項目，細節見〈待 Amber 決定 / 下一步〉。

動手前不需要重新討論預支款模型——上個 session 的收斂已經做完、驗證過、也推上 `origin/main` 了。

**DB 裡刻意留了一組 fixture**（見〈開發環境狀態 / 現有資料〉），其中王主管那張報銷單就是階段 5 要用的「別人的單」對照組，別急著刪。
...
- 桌機本機 DB 已套用最新 migration `20260806105024_RemoveCashAdvanceUsage`。
...
- `cash-advance-reconciliation.md`（含〈為什麼零用金不是預支款〉，是這次收斂的完整理由出處）
...
- 列表篩選目前先在 Application Service 對 `ListAsync()` 結果做 in-memory 篩選。資料量變大時，可新增查詢專用 repository method，把篩選下推到 EF Core / PostgreSQL。**預支款的可見度過濾（`CanBeViewedBy`）目前也是 in-memory**，同理。階段 5 的報銷單過濾屆時會是第三個。
...
**現有資料（2026-08-06 清空重建）**

`expense_reports`（含明細、審核紀錄）、`cash_advances`（含結清紀錄）、`projects` 已全部清空，**Identity 的使用者與角色保留**（王主管 / 陳員工兩個帳號，不用重設密碼）。

清空前的完整備份在 session scratchpad（repo 外，不進 Git）。**那份備份只能當歷史檔案看，還原不回來**——`payment_method` 存的是字串，enum 改名後舊值對不上；這也正是當初選擇清空而非寫資料遷移的理由。

清空後 Amber 手動測試留下一組 fixture，**刻意保留**：

- 預支款 1 筆：王主管 / 專案用款 / 1,000
- 報銷單 1 張：`6月第一週-1` / 王主管 / Submitted / 個人預支 / 890 / 1 筆明細，綁上面那筆預支款

用途有兩個：階段 5 需要一張「別人的單」來驗證員工看不到；890 對 1,000 核准後會產生 110 差額，順手是核對與結清流程的 fixture。
...
5. ⬜ **員工只看自己的報銷單**——列表與詳情依 `ApplicantUserId` 過濾，主管看得到全部（**下一個 session 的主線**）
6. ⬜ 帳號自助註冊與使用者管理——細節見下方

**階段 5 動手前先想清楚的**：`ApplicantUserId` 為 null 的舊資料已隨清空消失，所以不必再處理「沒有申請人的單要給誰看」。過濾落點比照 `CashAdvanceAppService.CanBeViewedBy(...)`（Application Service、in-memory），詳情頁記得也要擋，不能只擋列表。
...
> 刻意沒有跟預支款收斂併著做：收斂是重構（把錯的模型改對），這是新流程（在 `ExpenseReport` 上開新的狀態轉換），性質不同，混做會分不清問題出在哪。
