...
> 最後更新：2026-08-07 20:20 — 階段 6「帳號自助註冊與使用者管理」**已完成並 commit**，權限與帳號那條編號計畫（1–6）全部結束
...
不必重新討論帳號生命週期或使用者管理——階段 6 已經做完、六條流程都用真實 HTTP 測過、也 commit 了。

**環境注意**：桌機 DB 於 2026-08-07 全表清空重建，現在只有 `Admin` 與 `amber` 兩個帳號、**沒有任何業務資料**（見〈開發環境狀態 / 現有資料〉）。要驗證報銷或核對流程得先自己建。
...
- **帳號三態 `UserAccountStatus { Pending, Active, Disabled }`**
- **員工自助註冊**（登入頁有連結，`[AllowAnonymous]`）。註冊只能產生「員工 / 尚待啟用」，角色與狀態都寫死在 Application，不吃表單
- **登入先驗密碼、再判帳號狀態**。用 `CheckPasswordSignInAsync` 只驗密碼不發 cookie，保留登入失敗鎖定；通過後才 `SignInAsync`
- **使用者管理頁（限主管）**：啟用 / 停用 / 在員工與主管之間改角色 / 重設別人的密碼
...
- **Bootstrap 取代 seed**：角色每次開機確認；帳號只在 users 表為空時建一個主管 `Admin`，不再建 demo 帳號
...
- **`IUserAccountStore` + `UserAccountAppService`**——帳號的建立與維護（註冊、狀態、角色、密碼）。跟 `IUserDirectory` 同一個形狀但**刻意分成兩個介面**：那個是唯讀的下拉選單來源，這個會改帳號；合在一起的話「只想畫個領款人下拉」的地方也會拿到重設別人密碼的能力
...
- `user-accounts.md`（帳號本身：三態、登入順序、`IUserAccountStore` 為何獨立、緊急存取帳號、為什麼不做刪除使用者）
...
- **`UserAccountResult` 為什麼不是例外**：註冊時「帳號重複 + Email 重複 + 密碼太短」可能同時發生，Identity 本來就回一整包，用例外只能丟第一條；也順帶擋住 `IdentityResult` 漏進 Application。分界線是「使用者自己能修正的走 result 進 ModelState，違反業務規則的（最後一位主管、緊急帳號、找不到帳號）照舊丟例外」
...
- `IUserDirectory` / `IUserAccountStore` 放 `Application/Identity`、實作放 `Infrastructure/Identity`。`ListSelectableAsync` 只列 `Active`（不該把錢指派給尚待啟用或離職的人），`FindByIdAsync` 不過濾（帳號停用後歷史紀錄上的人還是要查得到）。
...
- **登入 / 登出 / 註冊 / 改密碼的 HTTP 端點留在 `AccountController`**：發 cookie 本來就是 Web 的事。但「帳號怎麼被建立與維護」的邏輯在 `UserAccountAppService`，Controller 只負責轉換與顯示。
...
### 帳號生命週期（2026-07-27 定案，2026-08-07 階段 6 實作完成）

- **註冊（自助）→ 啟用（主管）→ 用 → 停用（主管）**
- **員工自行註冊**，登入頁上有連結，`[AllowAnonymous]`。註冊只能產生員工，不能自封主管。
- **註冊後預設 `Pending`，主管啟用才能登入**。避免任何連得到主機的人直接就能用。
- **帳號狀態三態 `UserAccountStatus { Pending, Active, Disabled }`**——bool 分不出「還沒啟用過」和「離職被停用」，登入訊息會講錯。
- **登入先驗密碼、再判帳號狀態**，狀態訊息才講明白（「尚待主管啟用」/「已停用」）。順序相反的話，不用知道密碼就能靠訊息差異探測帳號是否存在（account enumeration）。
...
- 選 bootstrap 而不是「第一個註冊的人自動變主管」的理由：bootstrap 在第一次啟動時就完成，那時還沒人能連進來註冊，**不存在搶註冊的空窗期**；行為也可預測。
...
- **不能停用、不能降成員工**，連主管也不行。判斷依據是帳號上的 `IsProtected` 旗標，**不是比對帳號名稱**——名字可以改、也可能有人另外註冊同名帳號，靠字串認太脆。
...
DB 於 2026-08-07 把 13 張表全 `TRUNCATE`（`__EFMigrationsHistory` 保留），目的是讓 Amber 看得到「空系統 → bootstrap → 註冊 → 啟用」這條在有 fixture 時永遠看不到的路。

目前只有兩個帳號、**沒有任何業務資料**（沒有報銷單、預支款、專案）：

- `Admin` / 管理者 / Active / 主管 / **受保護（緊急存取帳號）**，密碼是 `Identity:SeedPassword`
- `amber` / 王主管 / Active / 主管，密碼 `AmberPass123`（測試過程註冊出來的，可自行改掉）
...
- **測試腳本不要從列表頁的 HTML 推測「第幾列是誰」。** 使用者列表刻意把「尚待啟用」排在最前面，順序會隨狀態改變；靠位置抓 UserId 會抓錯人（這個坑在階段 6 踩了兩次，一次把測試帳號誤停用、一次整輪測到錯的帳號）。**要嘛直接查 DB 拿 ID，要嘛從 HTML 用姓名對應。** 另外 PowerShell 的 `Where-Object` 只回一個結果時是純量不是陣列，`[0]` 會取到字串的第一個字元——要寫成 `@(...)[0]`。
...
6. ✅ **帳號自助註冊與使用者管理**——三態、自助註冊、登入先驗密碼再判狀態、使用者管理頁（啟用 / 停用 / 改角色 / 重設密碼）、修改密碼頁、bootstrap 取代 seed，外加緊急存取帳號與登入告警
...
- **本機領先 `origin`**：階段 6 的兩個 commit 尚未 push。
