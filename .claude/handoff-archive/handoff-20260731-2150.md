...
> 最後更新：2026-07-27 18:04 — 申請人 / 審核人 / 結清處理人改為自動記錄登入者
...
- **權限只做到「記錄是誰做的」，還沒做「限制誰可以做」**：`IsManager()` 目前唯一的用途是 Layout 的角色徽章，`ApplicantUserId` 只被寫入、還沒有任何程式讀它做判斷。
- 還沒限制誰可以核准 / 退回 / 拒絕，也還沒限制誰可以建預支款 / 結清 / 標記不採用。
- **任何登入者都能修改 / 送審別人的草稿**，沒有「只有申請人能動自己的單」這層檢查。
...
- 桌機本機 DB 已套用最新 migration `20260727091350_AddActorUserIds`。
...
- `identity-and-authentication.md`（已含「是誰做的」怎麼流進 Domain；**授權規則做完後要回來再補一段**）
...
- **操作者才自動帶入登入者，當事人不會**。預支款的**領款人（`PayeeName`）維持手動輸入**——它是「錢給了誰」，不是「誰在操作系統」，主管很可能替員工建立預支款。
- **申請人 = 建立報銷單的人**，建立後不可修改。
...
- dev 資料中有一張驗證階段 3 用的測試報銷單，標題以「（測試）」開頭。系統沒有刪除報銷單的功能，所以它會一直留著，不是資料異常。
...
### 進行中：權限與帳號工作（分 6 階段，已完成 1–3）

1. ✅ Identity 基礎建設
2. ✅ 登入 / 登出
3. ✅ 把手動輸入的人名改成登入者——申請人、審核人、結清處理人、不採用處理人
4. ⬜ **授權規則**——限制「誰可以做」，範圍有三塊：
   - 主管才能核准 / 退回 / 拒絕
   - 主管才能建預支款、登記結清、標記不採用
   - **只有申請人能修改 / 送審自己的草稿**（2026-07-27 納入本階段）
5. ⬜ 員工只看自己的報銷單——列表與詳情依 `ApplicantUserId` 過濾，主管看得到全部
6. ⬜ 帳號自助註冊與使用者管理——細節見下方

### 階段 6：帳號自助註冊與使用者管理

> 原本卡在階段 4 的「誰能新增 / 停用帳號」，2026-07-27 討論後獨立成一階段：階段 4、5 是改既有程式的判斷邏輯，這階段是新畫面，混在一起會讓階段 4 變得太大。
