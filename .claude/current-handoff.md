...
> 最後更新：2026-08-08 18:17 — **自助註冊已拿掉，改由主管建立帳號**，帳號狀態同時由三態收斂為兩態，已完成、已手動驗過

---

## ⚠️ 下一個 session 開場要做的事

**先問 Amber 要走哪一條**，不要自己挑。編號計畫已經走完，剩下的三條彼此獨立：

1. **主管自我核准缺口**——見〈已知缺口〉第一條。**卡的是制度不是技術**，動手前要先陪 Amber 決定「這間公司只有一位主管，他自己的單怎麼辦」。
2. **已核准報銷單更正 / 取消核准**——Amber 2026-08-05 兩度提起，是新流程不是重構，動手前要先做設計討論（見〈下一個設計主題〉）。
3. **預支款已結清後是否鎖定修改**——範圍最小，設計問題也最集中（見〈暫緩項目〉）。

帳號那一塊不必重新討論：階段 6 做完之後，2026-08-08 又把自助註冊整個拿掉、改成主管建帳號（理由見〈已定案的設計決策 / 帳號生命週期〉的推翻紀錄）。**不要提議把自助註冊做回來**，那個兩難沒有 SMTP 就解不掉。

**環境注意**：桌機 DB 於 2026-08-07 全表清空重建，現在有四個帳號、**沒有任何業務資料**（見〈開發環境狀態 / 現有資料〉）。要驗證報銷或核對流程得先自己建。
...
- **主管可以核准自己的報銷單**（2026-08-06 發現，尚未修）。`ExpenseReport.Approve()` 只檢查狀態是不是 Submitted，`ExpenseReportAppService.ApproveAsync` 只靠 Controller 的 `[Authorize(Roles = Manager)]` 擋角色，兩層都沒有比對「審核人 ≠ 申請人」。所以主管可以自己建單、自己送審、自己核准、再自己登記結清，全程沒有第二個人經手。這是內控缺口，**不是 bug——這條規則從來沒被寫過**。
...
後三個都是測試過程留下來的，可自行改掉或停用（系統刻意沒有刪除使用者的功能）。

**要驗證報銷或核對流程，得自己從頭建**：專案、預支款、報銷單、結清紀錄。特別是**目前沒有任何「專案支出」報銷單**，做已核准報銷單更正流程時要先建一張。
...
**6 之後的修正（2026-08-08）**：✅ 拿掉自助註冊改成主管建帳號、帳號狀態收斂為兩態。理由見〈已定案的設計決策 / 帳號生命週期〉。
...
**注意**：DB 已清空，做這題之前要先自己建一組 fixture（專案 + 專案支出報銷單 + 預支款 + 已核准的單 + 結清紀錄）。
...
- **只剩一位主管時給提示**：目前「至少一位啟用中的主管」只在你想拿掉他的當下才擋。要不要在使用者管理頁平時就顯示警告（提醒指派第二位主管），Amber 尚未決定。
