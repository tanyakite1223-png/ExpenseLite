...
2026-08-16 加了兩條並列規則：**不能停用自己的帳號、不能把自己降成員工**。順序上放在「最後一位主管」檢查之前，訊息更精確（撞到的人第一時間看到的是「不能對自己做」而不是「不能拿掉最後一位」）。

理由跟業界 IAM 慣例（Google Cloud、Azure、Windows admin console）一樣：**主管動自己幾乎沒有正當使用情境，誤按代價高**——停用完自己的 cookie 30 分鐘內失效，得請另一位主管重新啟用；把自己降成員工也一樣，馬上失去管理權限。要離職或交出主管職，改由另一位主管操作有更清楚的稽核意義（「誰讓他離開的」有紀錄）。

跟緊急存取帳號那條同一類——**「授權判斷」與「業務規則」要分清楚**：主管本來就有停用 / 改角色的角色權限，只是這一次資料不合法，所以丟 `DomainRuleViolationException` 而不是 `ForbiddenOperationException`。View 上 `isSelf` 那一列不畫「停用」與「改為員工」按鈕，但真正的把關在 `UserAccountAppService`（已用直接 POST 驗過）。「改為主管」不擋，因為升成主管不會造成該類鎖死。

### 只剩一位日常主管時的提示

「至少一位啟用中的主管」擋的是「動作發生的當下」——你想拿掉他時才會被擋。但**2026-08-08 加了「審核人 ≠ 申請人」之後多了一種新的鎖死**：只剩一位主管，他自己送的報銷單就沒人能審或作廢。這個問題不是操作被擋，是**流程走不動**，動作發生時系統無從察覺。

所以 2026-08-16 在使用者管理頁加了一條**平時就顯示的紅色 alert**（`UserAccountAppService.CountActiveDailyManagersAsync` + Razor 判斷 `<= 1`）——0 位、1 位分別顯示不同訊息，提醒主管去指派第二位。

**「日常主管」的定義**：`Status == Active && Role == Manager && !IsProtected`——**扣掉緊急存取帳號**。理由是那個帳號的設計理念就是「日常不用」，靠它撐日常審核違反該帳號定位；如果算它，只要 `Admin` 存在就永遠不會觸發提示，這個提示等於沒用。定義集中在 App Service 一處，View 只判斷「數字要不要觸發顯示」，不重寫規則。

**為什麼用 `alert-danger`（紅色）而不是 `alert-warning`（黃色）**：這條的觸發條件是「流程已經卡住 / 快要卡住」，不是「你可能想注意一下」——「1 位」意味著他自己送單就無法審，「0 位」意味著沒人能審任何單。跟業界 dashboard 對「系統可用性快掛掉」的視覺處理一致。
