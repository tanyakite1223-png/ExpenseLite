...
```
自助註冊 ──> Pending ──主管啟用──> Active ──主管停用──> Disabled
                                    ^                      |
                                    └──────主管啟用────────┘
```

原本 `ApplicationUser` 上是一個 `IsActive` bool。換成 `UserAccountStatus { Pending, Active, Disabled }` 的理由很具體：**bool 分不出「還沒被啟用過」和「用過但被停用」**，登入被擋下來時就講不出正確的話——該說「請等主管啟用」的時候說成「帳號已停用」，使用者會以為自己被開除了。
...
// 1. 帳號不存在 → 「帳號或密碼不正確」
// 2. 密碼錯 → 「帳號或密碼不正確」（同一句話）
// 3. 密碼對、但狀態不是 Active → 「尚待主管啟用」/「已停用」
// 4. 密碼對、狀態 Active → 發 cookie
```

如果把狀態判斷提到密碼前面，任何人都能在登入頁輸入一個帳號加隨便的密碼，靠回應的差異（「尚待啟用」vs「帳號或密碼不正確」）判斷這個帳號**是否存在**。那是帳號列舉（account enumeration），等於免費送出公司的員工名單。
...
1. **一次可能有好幾條錯誤**。註冊時帳號重複、Email 重複、密碼太短可以同時發生，Identity 本來就回一整包。用例外表達只能丟第一條。
...
差別在於 seed 是「維持某個狀態」，bootstrap 是「讓一個空系統有辦法被登入第一次」。之後的帳號一律走註冊 → 啟用。

為什麼選 bootstrap 而不是「第一個註冊的人自動變主管」：bootstrap 在第一次啟動時就完成，那時還沒有人連得進來註冊，**不存在搶註冊的空窗期**；行為也可預測。
