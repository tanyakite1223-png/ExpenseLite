...
### 也不是授權,是報銷單自己的規則:審核人 ≠ 申請人

主管本來就有權限核准 / 退回 / 拒絕(`[Authorize(Roles = Manager)]` 已擋在 Controller)——只是**同一個人不能同時當申請人與審核人**。用上面的分辨方法:換另一位主管來,他審別人的單一樣合法、審自己的單一樣不合法——答案跟角色無關,只跟「這張單的申請人是不是他自己」有關。這是報銷單自己的 invariant:一次合法的審核事件必須有兩位當事人,否則審核紀錄就是壞的(自己蓋章給自己)。

所以落點跟「只有 Submitted 才能被審核」一樣:

- 放在 `ExpenseReport.EnsureReviewerIsNotApplicant`(Domain entity 自己),不是在 Application Service
- 丟 `DomainRuleViolationException`,不是 `ForbiddenOperationException`——跟「緊急存取帳號不能停用」同一類,主管有權做這件事,只是這一次資料不合法

畫面上申請人自己看不到那三顆按鈕(`Details.cshtml` 的 `canReview` 排除了 `isApplicant`),但真正擋住的是 Domain 這條 invariant。這是目前少數幾個 `DomainRuleViolationException` 同時也擔任「防止越權」角色的例子——因為「越權」的判定條件剛好等於「資料不合法」的判定條件,兩層合而為一。
...
| 核准 / 退回 / 拒絕 | 只有主管,**且審核人不能是申請人本人**(見〈也不是授權,是報銷單自己的規則〉) |
...
| 建立 / 結案專案 | 只有主管 |
| 建立帳號 | 只有主管。**沒有自助註冊**（2026-08-08 拿掉，理由見〈[帳號生命週期與使用者管理](user-accounts.md)〉） |
| 使用者列表、啟用 / 停用 / 改角色 / 重設別人的密碼 | 只有主管（整個 `UsersController` 標在類別上） |
| 停用或降級**緊急存取帳號** | **沒有人可以**，連主管也不行（見〈[帳號生命週期與使用者管理](user-accounts.md)〉）。重設它的密碼則不受限——那是外流時的處置方式 |
| 修改自己的密碼 | 每個登入者改自己的，要驗舊密碼 |

使用者管理整塊都是純角色授權，沒有任何「同一份資料、不同人看到不一樣」的情況，所以 `[Authorize(Roles = Manager)]` 標在 controller 類別上就夠了，不需要往 Application Service 下沉。唯一需要 Application Service 判斷的是「不能拿掉最後一位啟用中的主管」——但那是業務規則不是授權（主管本來就有權限做這件事，只是這一次不合法），理由見〈[帳號生命週期與使用者管理](user-accounts.md)〉。
